...
function Home() {
  ...
  useEffect(() => {
    const fetchCredentials = async () => {
      try {
        ...
        if (data.status === 'success') {
          setCredentials(data.data || []);
          setError(data.data.length === 0 ? 'No data available' : null);
        } else {
          setError(data.message);
        }
      } catch (error) {
  ...
  const handleSelectChange = (event) => {
    ...
    if (['Single Credentials', 'Multiple Credentials'].includes(selectedValue)) {
      setModalContent(selectedValue);
      setModalIsOpen(true);
    } else if (selectedValue === 'Create Credentials') {
      navigate('/Groups');
  ...
  // Handle checkbox state for credential selection
  const handleCheckboxChange = (id) => {
    setSelectedCredentials((prevSelected) =>
  ...
  // Handle deletion of selected credentials
  const handleDelete = () => {
    setCredentials((prevCredentials) =>
      prevCredentials.filter((credential) => !selectedCredentials.includes(credential.Credential_ID))
    );
