...
function CredentialButtons() {
    ...
    return (
        <div className="flex flex-col items-center h-screen bg-cover bg-center bg-no-repeat bg-gray-100" style={{ backgroundImage: 'url(/background0.jpg)' }}>
            ...
            <a href="#" className="mt-5 text-blue-500 text-lg" onClick={() => setShowForm(prev => !prev)}>
                {showForm ? 'Cancel' : 'Add New Credential'}
            </a>
            {showForm && (
                <form onSubmit={handleFormSubmit} className="mt-5 flex flex-col items-center p-5 border border-green-700 rounded bg-white max-w-md w-full">
                    ...
                    <button type="submit" className="mt-3 px-4 py-2 bg-green-700 text-white font-semibold rounded hover:bg-green-800">
                        Add Credential
                    </button>
                </form>
            )}
